...
/**
 * Voidling - A small hostile End mob with GeckoLib animations.
 */
public class Voidling extends Monster implements GeoEntity {

    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public Voidling(EntityType<? extends Voidling> type, Level level) {
        super(type, level);
    }
    ...
    private PlayState movementController(AnimationState<Voidling> state) {
        if (state.isMoving()) {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.VOIDLING_AMBIENT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(@NotNull DamageSource damageSource) {
        return ModSounds.VOIDLING_HURT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.VOIDLING_DEATH.get();
    }

    protected SoundEvent getAttackSound() {
        return ModSounds.VOIDLING_ATTACK.get();
    }
}
